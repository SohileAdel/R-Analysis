...
for (i in rnds) {
  ...
  Cashes[i] <-
    sum(filter(data, rnd == i, paymentType == "Cash")$total)
  Credits[i] <-
    sum(filter(data, rnd == i, paymentType == "Credit")$total)
}
...
for (i in 1:length(Cities)) {
  totalCities[i] <-
    sum((filter(data_set, city == Cities[i])$total) / 1000)
}

sCities <- data.frame(city = Cities
                      , cTotal = totalCities)
...
pie(
  #we realize that cash and credit are equivalent
...
spendingInEachAge <-
  summarise(spendingInEachAge, totalSpending = sum((total / 1000)))
...
spendingTable <- data.frame(city = spendingArranged$city,
...
while (valid == F) {
  number_of_cluster <-
    as.numeric(readline("Enter The Number of Clusters between 2 and 4: "))
...
while (valid == F) {
  min_support <-
    as.numeric(
      readline(prompt = "Enter the minimum support (note:the minimum support must be between[1 and 100]%): ")
    )
  if (min_support < 1) {
...
while (valid == F) {
  min_conf <-
    as.numeric(
      readline(prompt = "Enter the minimum confidence (note:the minimum support must be between[1 and 100]%): ")
    )
  if (min_conf < 1) {
